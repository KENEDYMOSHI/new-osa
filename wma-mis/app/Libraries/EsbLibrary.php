...
class EsbLibrary
{
    ...
    {
        $this->clientId = env("govesb.client-id");
        $this->clientSecret = env("govesb.client-secret");
        $this->clientPrivateKey = env("govesb.private-key");
        $this->esbPublicKey = env("govesb.esb-public-key");
        $this->esbTokenUrl = env("govesb.esb-token-url");
        $this->esbEngineUrl = env("govesb.esb-engine-url");
        $this->nidaUserId = env("govesb.nida-user-id");
        $this->signatureAlgorithm = "ECC";
        $this->format = "json";
    ...
    /**
     * @throws EsbLibraryException
     * @throws Exception
     */
    public function getEsbAccessToken()
    {
        if (!$this->clientId || !$this->clientSecret) {
            throw new EsbLibraryException("clientId or clientSecret is null");
        }
        $curl = curl_init();
        ...
        if (!array_key_exists("access_token", $esbTokenResponse)) {
            throw new EsbLibraryException("Could not get token from GovESB");
        }
    ...
    private function response(bool $success, $data, $format = null)
    {
        ...
        if (!$this->format) {
            throw new EsbLibraryException("Data format not set");
        }
        if ($this->format == DataFormat::json) {
    ...
    private function createXmlRequest($isPushRequest, $userId)
    {
        ...
        if ($this->requestBody) {
            if (!$this->_isValidXML($this->requestBody)) {
                throw new EsbLibraryException("Invalid xml");
            }
    ...
    private function createJsonRequest($isPushRequest, $userId)
    {
        ...
        if ($this->requestBody) {
            ...
            if (!is_array($this->requestBody)) {
                throw new EsbLibraryException("Invalid esbBody");
            }
        }
    ...
    private function createXmlResponse($isSuccess, $xmlString, $isAsyncResponse)
    {
        if (!is_string($xmlString)) {
            throw new EsbLibraryException("Invalid response body, expected xml string or message");
        }
    ...
    {
        if ($this->format == DataFormat::json) {
            $jsonPayload = EsbLibrary::jsonDecode($esbRequestBody);
            $signature = $jsonPayload["signature"];
//            $signedData = EsbLibrary::jsonEncode($data);
            $signedData = $this->getSignedPayload($esbRequestBody);
            $data = EsbLibrary::jsonDecode($signedData);
        } else {
    ...
    /**
     * @throws EsbLibraryException|DOMException
     */
    ...
    public function signPayload($payload, $privateKey = null)
    {
        ...
        $signature = "";
        if ($this->signatureAlgorithm == "ECC") {
    ...
    private function validateRequestParameters($apiCode, $format, $esbBody)
    {
        ...
        if (!$this->apiCode) {
            throw new EsbLibraryException("apiCode can not be null");
        }

        if (!$this->format) {
            throw new EsbLibraryException("format can not be null");
        }
    }
...
class EsbLibraryException extends Exception
{
