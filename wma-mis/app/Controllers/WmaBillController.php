...
class WmaBillController extends BaseController
{
    protected $billModel;
    protected $uniqueId;
    protected $managerId;
    ...
    public function __construct()
    {
        ...
        helper(['recon','qr','text','setting','auth','bill','wmaBill','xml','form','inflector','alert','array','prePackage','url', 'date','image','regions']);
        ...
        try {
            if ($this->request->getMethod() == 'POST') {
                ...
                $itemsArray = [
                    ...
                    'RequestId' => fillArray($count, $BillId),
                ...
                $xml = Array2XML::createXML('BillItems', ['BillItem' => arrayExcept($items, ['center','BillId','RequestId', 'ItemName', 'UserId', 'Task', 'Status', 'ItemQuantity', 'SingleItemAmount', 'NextVerification', 'fob', 'tansardNumber', 'date'])])->saveXML();
                    ...
                    if ($code == '7101') {
                        ...
                        while ($attempt < $maxAttempts) {
                            ...
                            if (!empty($billRes)) {
                                ...
                                //check if the staPtus code returned to callback response code is 7101
                                if (strlen($gepgResponseCode) == 4 && $gepgResponseCode == '7101') {
                                    ...
                                    return $this->response->setJSON([
                                        ...
                                        'bill' => $bill->method == 'BankTransfer' ? transferNormalBill($billData) : normaSingleBill($billData),
    ...
    public function billReconciliation()
    {
        try {
            ...
            $this->sms->sendSms('255659851709', "User:  ($usr) has requested a Normal recon $date");
            $ReconcOpt = 1;
            $data = [
        ...
        try {
            if ($this->request->getMethod() == 'POST') {
                ...
                if ($submission->status == 1) {
                    ...
                    $response = XML2Array::createArray($submission->resultCurlPost);
                    $code = $response['Gepg']['gepgBillSubReqAck']['TrxStsCode'];
                   // $ackDesc = $response['Gepg']['gepgBillSubReqAck']['AckStsDesc'];
                    ...
                    if ($code == '7101' || $code == '7226') {
                        ...
                        while ($attempt < $maxAttempts) {
                            ...
                            if (!empty($billRes)) {
                                ...
                                $errCode =  substr($gepgResponseCode, 0, 4);
                                if (strlen($gepgResponseCode) == 4 && ($gepgResponseCode == '7101' ||  $errCode == '7226')) {
    ...
    public function selectBill()
    {
        ...
        if (!empty($bill)) {
            ...
            return $this->response->setJSON([
                'status' => 1,
                'token' => $this->token,
                'bill' => $method == 'BankTransfer' ? transferNormalBill($billData)  : normaSingleBill($billData),
                'qrCodeObject' =>   $qrCodeObject,
                'msg' => 'Bill Created Successfully',
    ...
    public function selectPayment()
    {
        ...
        if (!empty($payment)) {
            ...
            return $this->response->setJSON([
                'status' => 1,
                'receipt' => receiptView($receiptData),
