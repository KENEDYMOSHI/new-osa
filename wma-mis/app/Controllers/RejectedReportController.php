...
class RejectedReportController extends BaseController
{
    ...
    public function inspectedRejected()
    {
        ...
        // printer($instruments);
        // exit;
        return view('Pages/RejectedInspectionReport', $data);
    }
    ...
    public function filterInspectedRejected()
    {
        ...
        if ($month != '') {
            $title =  date("F", mktime(0, 0, 0, $month, 1)) . " " . $startYear . " Rejected Instruments Report";
        } elseif ($quarter != '') {

            $title =  "$period Rejected Instruments Report Of Financial Year " . $startYear . '|' . ($endYear) . ' ';
        } else {
            $title =  "$period Rejected Instruments  Report Of Financial Year " . date('Y', strtotime($initialDate)) . '|' . date('Y', strtotime($finalDate)) . ' ';
        }
    ...
    public function rejectionNote($name,$address,$deadline,$activity,$instrument){
      
        

        $orientation =  'P';
        $title = $name .  '-'.$instrument .'-' . microtime();
        $data['title'] = $title;
        $data['name'] = $name;
        ...
        $pdfLibrary = new PdfLibrary();
        $pdfLibrary->renderPdf(orientation: $orientation, view: 'Pages/Vtc/RejectionNote', data: $data, title: $title);
    }
}
